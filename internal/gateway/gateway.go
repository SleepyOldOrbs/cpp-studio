...
func (r *router) handleVoiceConversion(w http.ResponseWriter, req *http.Request) {
	...
	defer cleanupSource()
	if err := validateVoiceConversionWAV(sourcePath, "source"); err != nil {
		writeJSONError(w, http.StatusBadRequest, err.Error())
		return
	}
	...
	defer cleanupTarget()
	if err := validateVoiceConversionWAV(targetPath, "target"); err != nil {
		writeJSONError(w, http.StatusBadRequest, err.Error())
		return
	}
...
func validateVoiceConversionWAV(path string, label string) error {
	...
	if info.Size() > maxVoiceConversionWAVBytes {
		return fmt.Errorf("%s WAV is %d bytes, max is %d bytes", label, info.Size(), maxVoiceConversionWAVBytes)
	}
	return nil
}
...
func (r *router) musicSource(req *http.Request, required bool) (string, func(), error) {
	...
	if err := validateMusicSourceWAV(path); err != nil {
		cleanup()
		return "", func() {}, err
	}
...
func validateMusicSourceWAV(path string) error {
	if err := wav.ValidateFile(path); err != nil {
		return fmt.Errorf("source must be a valid WAV: %v", err)
	}
	info, err := os.Stat(path)
	if err != nil {
		return fmt.Errorf("stat source WAV: %v", err)
	}
	if info.Size() > maxMusicSourceWAVBytes {
		return fmt.Errorf("source WAV is %d bytes, max is %d bytes", info.Size(), maxMusicSourceWAVBytes)
	}
	return nil
}
...
func (r *router) handleMusicGeneration(w http.ResponseWriter, req *http.Request) {
	...
	sourcePath, cleanup, err := r.musicSource(req, musicRoutesRequiringSource[musicRequest.Route])
	if err != nil {
		writeJSONError(w, http.StatusBadRequest, err.Error())
		return
	}
...
func (r *router) handleMusicAnalysis(w http.ResponseWriter, req *http.Request) {
	...
	sourcePath, cleanup, err := r.musicSource(req, true)
	if err != nil {
		writeJSONError(w, http.StatusBadRequest, err.Error())
		return
	}
