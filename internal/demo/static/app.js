(function () {
  ...
  async function normalizeConversionUpload(kind, file) {
    ...
    var looksLikeWav = /\.wav$/i.test(name) || /wav/i.test(file.type || "");
    if (looksLikeWav) {
      setConversionFile(kind, file, "upload");
      return;
    }
    ...
    try {
      ...
      log("POST /v1/audio/decode " + name);
      var response = await fetch("/v1/audio/decode", { method: "POST", body: form });
      await ensureOk(response, "Audio conversion");
      var blob = await response.blob();
      setConversionFile(kind, new File([blob], name.replace(/\.[^.]+$/, "") + ".wav", { type: "audio/wav" }), "ffmpeg");
    } catch (error) {
  ...
  async function startConversionRecording(kind, event) {
    ...
    try {
      ...
      pending.audioContext = new window.AudioContext();
      pending.source = pending.audioContext.createMediaStreamSource(pending.stream);
      pending.processor = pending.audioContext.createScriptProcessor(4096, 1, 1);
      pending.chunks = [];
      pending.length = 0;
      pending.sampleRate = pending.audioContext.sampleRate;
      pending.kind = kind;
      pending.vu = kind === "source" ? conversionSourceVu : conversionTargetVu;
      pending.processor.onaudioprocess = function (audioEvent) {
        if (conversionCapture !== pending) {
          return;
        }
        ...
        pending.length += copy.length;
        updateVuInto(pending.vu, copy);
      };
      pending.source.connect(pending.processor);
      pending.processor.connect(pending.audioContext.destination);
  ...
  async function stopConversionRecording() {
    ...
    var blob = encodeWav(samples, current.sampleRate);
    setConversionFile(current.kind, new File([blob], current.kind + "-voice.wav", { type: "audio/wav" }), "microphone");
    syncControls();
  }
  ...
  async function normalizeMusicUpload(file) {
    ...
    try {
      var form = new FormData();
      form.append("file", file, name);
      log("POST /v1/audio/decode " + name + " for music");
      var response = await fetch("/v1/audio/decode", { method: "POST", body: form });
      await ensureOk(response, "Audio conversion");
      var blob = await response.blob();
      setMusicSource(new File([blob], name.replace(/\.[^.]+$/, "") + ".wav", { type: "audio/wav" }), "ffmpeg");
    } catch (error) {
  ...
  function syncMusicControls() {
    ...
    musicSourceInput.disabled = studioBusy || captureBusy;
    musicSourceRecordButton.disabled = studioBusy || (musicCapturePending && !musicCapture) || (!captureBusy && !canRecord());
  ...
  async function startMusicRecording(event) {
    ...
    try {
      ...
      pending.processor.onaudioprocess = function (audioEvent) {
        ...
        pending.chunks.push(copy);
        pending.length += copy.length;
        updateVuInto(musicSourceVu, copy);
      };
      pending.source.connect(pending.processor);
      pending.processor.connect(pending.audioContext.destination);
  ...
  async function stopMusicRecording() {
    ...
    musicSourceRecordButton.classList.remove("recording");
    musicSourceRecordButton.textContent = "Hold to record";
    resetVuInto(musicSourceVu);
    await cleanupRecorderResources(current);
    var samples = mergeChunks(current.chunks, current.length);
    if (samples.length < Math.floor(current.sampleRate / 4)) {
      syncControls();
      setMusicError(new Error("The music source recording is too short"));
      return;
    }
    var blob = encodeWav(samples, current.sampleRate);
    setMusicSource(new File([blob], "music-source.wav", { type: "audio/wav" }), "microphone");
    syncControls();
  }
  ...
  // --- Studio navigation: formats -> tools -----------------------------
  // Parent links open an overview desk; the second row holds that format's
  // tools. Management pages remain separate because they support every desk.
  var PAGES = [
  ...
  var devScratchpad = document.getElementById("devScratchpad");
  var devDvdLogo = document.getElementById("devDvdLogo");
  var devScreensaverFrame = 0;
  var devScreensaverLastTime = 0;
  var devScreensaverActive = false;
  var devDvdX = 18;
  var devDvdY = 18;
  var devDvdVelocityX = 92;
  var devDvdVelocityY = 74;
  var devDvdColor = 0;
  var DEV_DVD_COLORS = ["#f0a64b", "#63d1cb", "#bd8cdf", "#58c97c", "#df8275", "#6fa8ff"];

  function positionDevDvd() {
    devDvdLogo.style.transform = "translate3d(" + Math.round(devDvdX) + "px, " + Math.round(devDvdY) + "px, 0)";
  }

  function stepDevScreensaver(time) {
    if (!devScreensaverActive) {
      return;
    }
    if (!devScreensaverLastTime) {
      devScreensaverLastTime = time;
    }
    var elapsed = Math.min((time - devScreensaverLastTime) / 1000, 0.05);
    devScreensaverLastTime = time;
    var maxX = Math.max(0, devDvdLogo.parentElement.clientWidth - devDvdLogo.offsetWidth);
    var maxY = Math.max(0, devDvdLogo.parentElement.clientHeight - devDvdLogo.offsetHeight);
    devDvdX += devDvdVelocityX * elapsed;
    devDvdY += devDvdVelocityY * elapsed;
    var bounced = false;
    if (devDvdX <= 0 || devDvdX >= maxX) {
      devDvdX = Math.max(0, Math.min(maxX, devDvdX));
      devDvdVelocityX *= -1;
      bounced = true;
    }
    if (devDvdY <= 0 || devDvdY >= maxY) {
      devDvdY = Math.max(0, Math.min(maxY, devDvdY));
      devDvdVelocityY *= -1;
      bounced = true;
    }
    if (bounced) {
      devDvdColor = (devDvdColor + 1) % DEV_DVD_COLORS.length;
      devDvdLogo.style.color = DEV_DVD_COLORS[devDvdColor];
    }
    positionDevDvd();
    devScreensaverFrame = window.requestAnimationFrame(stepDevScreensaver);
  }

  function setDevScreensaverActive(active) {
    devScreensaverActive = Boolean(active && devScratchpad.dataset.empty === "true");
    window.cancelAnimationFrame(devScreensaverFrame);
    devScreensaverFrame = 0;
    devScreensaverLastTime = 0;
    if (devScreensaverActive) {
      devScreensaverFrame = window.requestAnimationFrame(stepDevScreensaver);
    }
  }
  ...
  function applyPage(name) {
    ...
    setDevScreensaverActive(name === "dev");
  ...
  function renderLibraryVoice(voice) {
    ...
    return searchableLibraryCard(row, [voice.name, voice.transcript, voice.id, analysis.fitness, voice.source && voice.source.name]);
  }
